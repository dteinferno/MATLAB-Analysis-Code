...
% Also save images and matrices of the mean MIPs before and after
% registration and of the mean planes
...
for dirNow = 1:numDirs
    ...
    for tifID = 3:length(fileNames)
        ...
        if strcmp(tifName(end-3:end),'.tif') & ~contains(tifName,'Anatomy')
            ...
            save(strcat(fName,'_RegDat.mat'), 'Px','regCoords','meanRegStack','planeAverage');
            ...
            for planeNow = 1:num_planes-num_discards
                subplot(3,4,4+planeNow);
                imagesc(squeeze(planeAverage(planeNow,:,:)));
                axis equal;
                axis off;
                title(strcat('ave plane #',num2str(planeNow)));
            end
...
%% Set the ROIs and the cut planes
...
for dirNow = 1:numDirs
    ...
    for tifID = 3:length(fileNames)
        ...
        if strcmp(tifName(end-3:end),'.tif') & ~contains(tifName,'Anatomy')
            ...
            cutROIs = zeros(size(planeAverage));
            for planeNow = 1:num_planes-num_discards
                planePic = figure;
            ...
            save(strcat(fName,'_cutROIs','.mat'), 'cutROIs');
            clear cutROIs;
        end
    end 
end

%% For each stack perform the following operations: cut, register, calculate ROIs
