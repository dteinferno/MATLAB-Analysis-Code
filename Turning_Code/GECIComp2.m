...
if invert
    ...
    % Alternative time constants taken from Fig 2
    RtOn = 0.046/log(2);
    RtOff = 0.35/log(2);
    % Time constants taken from Ultrasensitive fluorescent proteins for imaging neuronal activity 
    GtOn = 0.13/log(2);
    GtOff = 0.37/log(2);
end
...
%             GbumpShape = 0.5*cos(ROIs(ROI)-bumpShift-bumpSpeed*tAll)+0.5;
        ...
        if ( velBump == 2 || velBump == 7 || velBump == 12 ) & shft == 2
            ...
            RBPlot = Rbump(:,floor(length(tAll/2))-1)./max(Rbump(:,floor(length(tAll/2))-1));
            RBMax = find(RBPlot == max(RBPlot));
            RBPlot(1:end-1) = circshift(RBPlot(1:end-1),8-RBMax);
            RBPlot(end) = RBPlot(1);
            GBPlot = Gbump(:,floor(length(tAll/2))-1)./max(Gbump(:,floor(length(tAll/2))-1));
            GBPlot(1:end-1) = circshift(GBPlot(1:end-1),8-RBMax);
            GBPlot(end) = GBPlot(1);
            %             plot(ROIs,0.5*cos(ROIs-bumpSpeed*tAll(round(length(tAll/2))))+0.5,'k');
            plot(ROIs,circshift(exp(-(mod((ROIs-bumpSpeed*tAll(floor(length(tAll/2))-1)),2*pi)-pi).^2/(2*(FWHM/(2*sqrt(2*log(2))))^2))',8-RBMax),'k');
%             plot(ROIs,0.5*cos(ROIs-bumpShift-bumpSpeed*tAll(round(length(tAll/2))))+0.5,'k');
            plot(ROIs,circshift(exp(-(mod((ROIs-bumpShift-bumpSpeed*tAll(floor(length(tAll/2))-1)),2*pi)-pi).^2/(2*(FWHM/(2*sqrt(2*log(2))))^2))',8-RBMax),'k');
        ...
        RturnsR = Rbump(:,floor(length(tAll/2))-1)./max(Rbump(:,floor(length(tAll/2))-1));
        RturnsG = Gbump(:,floor(length(tAll/2))-1)./max(Gbump(:,floor(length(tAll/2))-1));
...
if invert
    print(GECIComp,'D:\Imaging\2Color\GECIComp_Split','-dpdf');
else
    print(GECIComp,'D:\Imaging\2Color\GECIComp_Split_Vert','-dpdf');
end