...
% Get the reference ROIs for each flies
...
% RstackRegMax = imReg(RstackXYfiltBGsubMax,10,10);
% GstackRegMax = imReg(GstackXYfiltBGsubMax,10,10);
% RstackRegMean = imReg(RstackXYfiltBGsubMean,10,10);
% GstackRegMean = imReg(GstackXYfiltBGsubMean,10,10);
RstackRegMax = RstackXYfiltBGsubMax;
GstackRegMax = GstackXYfiltBGsubMax;
RstackRegMean = RstackXYfiltBGsubMean;
GstackRegMean = GstackXYfiltBGsubMean;

%% Extract the ROIs and find the DF/F
% [ROIs, RROIaveREFMax] = GeneralROIS(RstackXYfiltBGsubMax);
% [ROIs, GROIaveREFMax] = GeneralROIS(GstackXYfiltBGsubMax);
[ROIs, GROIaveREFMax] = GeneralROIS(GstackRegMax);
...
print(act,strcat(imagePathname,imageFilename(1:end-4),'_BumpAct'),'-dpdf'); 
...
print(RGcomp,strcat(imagePathname,imageFilename(1:end-4),'_Summary'),'-dpdf'); 
...
writerObj = VideoWriter(strcat(imageFilename(1:end-4),'_XC.avi'));
...
for i=1:size(RturnsG,2)
    ...
    RturnsG(:,i) = circshift(RturnsG(:,i),round(num_ROIs/2)-pkPos);
    RturnsR(:,i) = circshift(RturnsR(:,i),round(num_ROIs/2)-pkPos);
end
for i=1:size(LturnsG,2)
    pkPos = find(LturnsG(:,i)==max(LturnsG(:,i)));
    LturnsG(:,i) = circshift(LturnsG(:,i),round(num_ROIs/2)-pkPos);
    LturnsR(:,i) = circshift(LturnsR(:,i),round(num_ROIs/2)-pkPos);
end
...
hold on;
plot(mean(RXCAlign,2),'r')
plot(mean(GXCAlign,2),'g')
axis off;
title('Mean peak shapes');
