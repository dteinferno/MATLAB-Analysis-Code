...
function [fullpath, Rstack, Gstack, positionDat] = VRImDatLoadDiscardNoPromptOnePlane(imageFilename,imagePathname,posFilename,posPathname,numFiles,planeNum)
...
for fID=1:numFiles
    for incIm = 1:num_images_ind(fID)
        ...
        if mod(2*num_planes+(incIm+imOffset-2*planeNum),2*num_planes) == 0
            Rstack(:,:,ceil((incIm+imOffset)/(2*num_planes))) = double(imread(fullpath{fID}, incIm-1, 'Info', info{fID}));
